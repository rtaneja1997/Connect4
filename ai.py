...
def play_genius(board): 
	...
	best_move=minimax_ab(board,-INFINITY, INFINITY, 1, DEPTH) 
	return best_move.move 
...
def minimax_ab(board, a, b, turn, depth): 
	...
	if turn==1:
		...
		for move in possible_moves:
			...
			move_score=minimax_ab(new_board, a, b, (turn+1)%2, depth-1).score 
			value=max(value, move_score) 
			a=max(a,value) 
			if a>=b:
				break 
		return MoveInfo(move, value) 
